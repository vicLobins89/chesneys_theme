				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						case 'label':
							...
							$value .= '<span>'.$grouped_product_child->get_sku().'</span>';
							$value .= '</label>';
							break;
						case 'desc':
							$value  = '<label for="product-' . esc_attr( $grouped_product_child->get_id() ) . '">';
							$value .= $grouped_product_child->is_visible() ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', $grouped_product_child->get_permalink(), $grouped_product_child->get_id() ) ) . '">' . $grouped_product_child->get_description() . '</a>' : $grouped_product_child->get_description();
							$value .= '</label>';
							break;
						case 'quantity':
							ob_start();
					...
					echo '<div class="counter">'.($key+1).'</div><td class="woocommerce-grouped-product-list-item__' . esc_attr( $column_id ) . '">' . apply_filters( 'woocommerce_grouped_product_list_column_' . $column_id, $value, $grouped_product_child ) . '</td>'; // WPCS: XSS ok.
